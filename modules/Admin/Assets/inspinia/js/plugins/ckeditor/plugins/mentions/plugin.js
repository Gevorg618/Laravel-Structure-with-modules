...
CKEDITOR_mentions.prototype.timeout_callback = function (args) {
  // Enable if this is on
  if(undefined == CKEDITOR.config.enableMentions || !CKEDITOR.config.enableMentions) {
    return;
  }
  var mentions   = args[0];
  ...
  $.getJSON(CKEDITOR.config.baseUrl + CKEDITOR.config.mentionsUrl, {typed: str}, function(rsp) {
    ...
    $('.mention-users').click(function(e) {
      ...
      link.href = 'mention:' + $(this).data('id');
      link.target = '_blank';
