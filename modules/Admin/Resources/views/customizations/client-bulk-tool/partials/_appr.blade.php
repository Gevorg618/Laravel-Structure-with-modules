                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="alert alert-info">
                            Add or Remove Appraisal Types from One, many or all clients.
                        </div>
                    </div>
                </div>
                <form class="form-horizontal" action="{{route('admin.management.client-bulk-tool.update')}}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="column" value="show_apprtype">
                    <div class="row">
                        <div class="col-md-6">
                                ...
                                <div class="col-md-10">
                                    <select name="add[]" class="form-control bootstrap-multiselect" multiple="multiple">
                                        @foreach($apprTypes as $apprType)
                                            <option value="{{$apprType->id}}">{{$apprType->getTitleAttribute()}}</option>
                                        @endforeach
                                    </select>
