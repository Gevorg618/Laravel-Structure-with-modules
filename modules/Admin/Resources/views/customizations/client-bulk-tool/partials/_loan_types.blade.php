                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="alert alert-info">
                            Add or Remove Loan Types from One, many or all clients.
                        </div>
                    </div>
                </div>
                <form class="form-horizontal" action="{{route('admin.management.client-bulk-tool.update')}}" method="POST">
                    {{ csrf_field() }}
                    <input type="hidden" name="column" value="show_loantype">
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <select name="add[]" class="form-control bootstrap-multiselect" multiple="multiple">
                                        @foreach($loanTypes as $loanType)
                                            <option value="{{$loanType->id}}">{{$loanType->descrip}}</option>
                                        @endforeach
                                    </select>
